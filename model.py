...
class VladPooling(keras.engine.Layer):
    ...
    def __init__(self, mode, k_centers, g_centers=0, **kwargs):
        self.k_centers = k_centers    #10
        self.g_centers = g_centers    #2
        self.mode = mode
    ...
    def call(self, x):
        ...
        A = K.expand_dims(A, -1)    # A : bz x W x H x clusters x 1 (?, 1, 16, 12, 1)
        feat_broadcast = K.expand_dims(feat, -2)    # feat_broadcast : bz x W x H x 1 x D (?, 1, 16, 1, 512)
        feat_res = feat_broadcast - self.cluster    # feat_res : bz x W x H x clusters x D (?, 1, 16, 12, 512)
        weighted_res = tf.multiply(A, feat_res)     # weighted_res : bz x W x H x clusters x D (?, 1, 16, 12, 512)
        cluster_res = K.sum(weighted_res, [1, 2])   # cluster_res = (?, 12, 512)

        if self.mode == 'gvlad':
            cluster_res = cluster_res[:, :self.k_centers, :]    # cluster_res = (?, 10, 512)

        cluster_l2 = K.l2_normalize(cluster_res, -1)
        outputs = K.reshape(cluster_l2, [-1, int(self.k_centers) * int(num_features)])  # outputs = (?, 5120)
        return outputs
